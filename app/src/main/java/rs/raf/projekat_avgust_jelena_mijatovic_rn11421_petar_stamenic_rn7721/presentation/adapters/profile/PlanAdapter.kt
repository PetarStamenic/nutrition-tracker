...
class PlanAdapter:RecyclerView.Adapter<PlanAdapter.PlanViewHolder>() {
    ...
    inner class PlanViewHolder(val binding: FragmentDailyPlanItemBinding):RecyclerView.ViewHolder(binding.root)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PlanViewHolder {
        return PlanViewHolder(FragmentDailyPlanItemBinding.inflate(LayoutInflater.from(parent.context),parent,false))
    }
    ...
    override fun onBindViewHolder(holder: PlanViewHolder, position: Int) {
        ...
        holder.binding.apply {
            ...
            dailyPlanItemKcal.setText(planItem.mealWithCalories.calculateCaloriesByMesurements().roundToInt().toString())
            DownloadImageFromInternet(dailyPlanItemThumb).execute(planItem.mealWithCalories.meal.thumbnail)
            dailyPlanItemDelete.setOnClickListener{
                var removed = planItems.toMutableList()
                removed.remove(planItem)
                planItems = removed
                fragment.viewModel.DeleteFromDatabase(planItem).execute()
            }
            dailyPlanItemThumb.setOnClickListener{
                ...
                fragment.activity?.supportFragmentManager?.beginTransaction()?.replace(R.id.meal_single_fragment,mealSingleFragment)?.commit()
                fragment.activity?.findViewById<TabLayout>(R.id.activity_main_tabLayout)?.getTabAt(2)?.select()
